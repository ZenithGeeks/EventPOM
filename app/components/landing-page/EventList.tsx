...
export default function EventList({ filteredEvents = [] }: EventListProps) {
    ...
    const groupedEvents = filteredEvents
        ...
        .reduce((acc: { [key: string]: any[] }, event: any) => {
            const category = event.category || "Uncategorized"; // Fallback if category is missing
            if (!acc[category]) {
                acc[category] = [];
            }
    ...
    return (
        <section className="flex items-center justify-center mt-12 px-4 sm:px-8 md:px-12 lg:px-16">
            <div className="w-full max-w-screen-xl">
                {Object.entries(groupedEvents).map(([category, events]: [string, any[]]) => (
                    <div key={category} className="mb-12">
                        {/* Category Name - Top Left */}
                        <h3 className="text-xl font-semibold text-indigo-900 mb-4">{category}</h3>
                        {/* Event Grid */}
