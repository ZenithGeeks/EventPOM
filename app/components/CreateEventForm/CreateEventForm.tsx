"use client";

import * as React from "react";
import PosterUpload from "./PosterUpload";
import EventDetailsForm from "./EventDetailsForm";
import TicketsForm from "./TicketsForm";
import { Button } from "@/components/ui/button";
import { useState } from "react";
import { Event } from "@/types/models"; // Import type definitions

export default function CreateEventForm() {
  const [date, setDate] = useState<Date>();
  const [picture, setPicture] = useState<File | null>(null);
  const [eventData, setEventData] = useState<Omit<Event, "id" | "createdAt">>({
    title: "",
    typeId: "",
    description: "",
    location: "",
    imageUrl: "",
    startTime: "",
    endTime: "",
    status: "PENDING_APPROVAL",
    organizerId: "123", // Replace with actual organizer ID
    tickets: [],
  });

  async function handleSubmit(event: React.FormEvent) {
    event.preventDefault();

    try {
      // Prepare event data
      const formattedData: Omit<Event, "id" | "createdAt"> = {
        ...eventData,
        startTime: date ? date.toISOString() : "",
        endTime: date ? new Date(date.getTime() + 2 * 60 * 60 * 1000).toISOString() : "",
        imageUrl: picture ? URL.createObjectURL(picture) : "",
      };

      // Send request to Next.js API
      const response = await fetch("/api/events", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(formattedData),
      });

      const result = await response.json();

      if (!response.ok) {
        throw new Error(result.error || "Failed to create event");
      }

      console.log("Event Created Successfully:", result.event);
      alert("Event created successfully!");

    } catch (error) {
      console.error("Event creation failed:", error);
      alert("Failed to create event.");
    }
  }

  return (
    <form className="flex flex-col gap-12 pt-10 px-4" onSubmit={handleSubmit}>
      <div className="flex flex-col md:flex-row gap-12">
        <div className="flex items-center justify-center md:justify-start">
          <PosterUpload picture={picture} setPicture={setPicture} />
        </div>
        <div className="flex-1">
          <EventDetailsForm date={date} setDate={setDate} />
          <TicketsForm />
        </div>
      </div>

      <div className="flex justify-end">
        <Button type="submit">Create Event</Button>
      </div>
    </form>
  );
}
