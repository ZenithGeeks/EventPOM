...
const SettingsComponent: React.FC<SettingsComponentProps> = ({ onSave, onDeleteAccount }) => {
  ...
  return (
    <div>
      {/* Header moved outside the box and made larger */}
      <h2 className="text-3xl font-bold text-[#2A2A6D] mb-4">User Settings</h2>
      <div className="p-6 pt-10 bg-white border shadow-md">
        ...
        <div className="flex flex-col">
          ...
          <div className="flex flex-col md:flex-row md:space-x-4 mb-4">
            <div className="md:w-1/2">
              ...
              <input type="text" defaultValue={session?.user?.name?.split(" ")[0] || "Johnny"} className="w-full p-2 border" />
            </div>
            <div className="md:w-1/2">
              <label className="block text-sm font-medium text-gray-700 uppercase mb-1">Last Name (as on ID Card or Passport)</label>
              <input type="text" defaultValue={session?.user?.name?.split(" ")[1] || "Doeman"} className="w-full p-2 border" />
            </div>
          </div>
          <div className="flex flex-col md:flex-row md:space-x-4 mb-4">
            <div className="md:w-1/3">
              <label className="block text-sm font-medium text-gray-700 uppercase mb-1">Email</label>
              <input type="email" defaultValue={session?.user?.email || "JohnDoe555@gmail.com"} readOnly className="w-full p-2 border bg-gray-100 cursor-not-allowed" />
            </div>
            <div className="md:w-1/3">
